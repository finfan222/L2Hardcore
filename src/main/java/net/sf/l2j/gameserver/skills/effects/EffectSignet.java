...
public class EffectSignet extends AbstractEffect {
    ...
    public boolean onActionTime() {
        ...
        final L2Skill signetSkill = SkillTable.getInstance().getInfo(((Signet) _skill).effectId, _skill.getLevel());
        final Creature[] targets = list.toArray(new Creature[list.size()]);
        for (Creature creature : targets) {
            signetSkill.applyEffects(_actor, creature);
