...
public class ScrollsOfResurrection implements IItemHandler {
    ...
    public void useItem(Playable playable, ItemInstance item, boolean forceUse) {
        ...
        if (target.isDead()) {
            ...
            if (targetPlayer != null || targetPet != null) {
                ...
                if (targetPlayer != null) {
                    ...
                    Dialog dialog = targetPlayer.getDialog();
                    if (targetPlayer.isReviveRequest() && dialog != null) {
                    ...
                    if (targetPet.getOwner() != player) {
                        Dialog dialog = targetPet.getOwner().getDialog();
                        boolean isRevivingPet = dialog.findAndGet("isRevivingPet");
                        if (targetPet.getOwner().isReviveRequest()) {
                            if (isRevivingPet) {
                                player.sendPacket(SystemMessageId.RES_HAS_ALREADY_BEEN_PROPOSED);
                            } else {
        ...
        if (skills == null) {
            log.warn("{} doesn't have any registered skill for handler.", item.getName());
            return;
        }
