...
/**
 * Allows spawning of a NPC object based on a timer (from the official idea used for the Merchant and Blacksmith of
 * Mammon).
 * <P>
 * General Usage:<BR> Call registerSpawn() with the parameters listed below:
 * <UL>
 ...
 * </UL>
 * spawnPoints is a standard two-dimensional int array containing X,Y and Z coordinates. The default respawn/despawn
 * delays are currently every hour (as for Mammon on official servers).</LI><BR>
 * <LI>The resulting AutoSpawnInstance object represents the newly added spawn index. - The interal methods of this
 * object can be used to adjust random spawning, for instance a call to setRandomSpawn(1, true); would set the spawn at
 * index 1 to be randomly rather than sequentially-based. - Also they can be used to specify the number of NPC instances
 * to spawn using setSpawnCount(), and broadcast a message to all users using setBroadcast(). Random Spawning = OFF by
 * default Broadcasting = OFF by default
 *
 * @author Tempy
 */
public class AutoSpawnTable {
    ...
    /**
     * Registers a spawn with the given parameters with the spawner, and marks it as active.<br> Returns a
     * AutoSpawnInstance containing info about the spawn.
     *
     * @param npcId
     * @param spawnPoints
    ...
    /**
     * Registers a spawn with the given parameters with the spawner, and marks it as active.<BR> Returns a
     * AutoSpawnInstance containing info about the spawn.<BR>
     * <BR>
    ...
    /**
     * Attempts to return the AutoSpawnInstance associated with the given NPC or Object ID type. <BR> Note: If
     * isObjectId == false, returns first instance for the specified NPC ID.
     *
     * @param id
     * @param isObjectId
     * @return AutoSpawnInstance spawnInst
    ...
    /**
     * AutoSpawner Class This handles the main spawn task for an auto spawn instance, and initializes a despawner if
     * required.
     *
     * @author Tempy
     */
