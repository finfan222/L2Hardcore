...
/**
 * Load and store {@link CursedWeapon}s. A cursed weapon is a feature involving the drop of a powerful weapon, which
 * stages on player kills and give powerful stats.
 * <ul>
...
public class CursedWeaponManager implements IXmlReader {
    ...
    /**
     * Check if a {@link CursedWeapon} can drop, verifying if it is already active and if the killed {@link Attackable}
    ...
    /**
     * Assimilate a {@link CursedWeapon} if the {@link Player} set as parameter already possesses one (which ranks up
     * possessed weapon), or activate it otherwise.
     *
     ...
     * <br>
     * If so, we set the {@link Player} and item references on the {@link CursedWeapon}, then we reward associated
     * skills to that {@link Player}.
     *
     * @param player : The {@link Player} to test.
     */
