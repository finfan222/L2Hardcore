...
public class Shutdown extends Thread {
     ...
     * shutdown hook and we save all data and disconnect all clients.<br>
     * <br>
     * After this thread ends, the server will completely exit if this is not the thread of getInstance, then this is a
     * countdown thread.<br>
     * <br>
     * We start the countdown, and when we finished it, and it was not aborted, we tell the shutdown-hook why we call
     * exit, and then call exit when the exit status of the server is 1, startServer.sh / startServer.bat will restart
     * the server.
     */
    @Override
    public void run() {
    ...
    /**
     * Report the current countdown to all players. Flag the server as "down" when reaching 60sec. Rehabilitate the
     * server status if ABORT {@link ServerStatus} is seen.
     */
