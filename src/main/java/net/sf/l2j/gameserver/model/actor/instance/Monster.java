...
public class Monster extends Attackable {
     ...
     * <br>
     * This state affects behaviors such as auto loot configs, Command Channel acquisition, or even Config related to
     * raid bosses.<br>
     * <br>
    ...
    /**
     * Register a {@link Player} into this instance _absorbersList, setting the HP ratio. The {@link AbsorbInfo} must
     * already exist.
     *
     * @param player : The {@link Player} to test.
     */
    ...
    private IntIntHolder calculateSweepSeedRewardItem(DropData drop, int levelModifier, boolean isSweep) {
        ...
        final int minCount = drop.getMin();
        final int maxCount = drop.getMax();
    ...
    public void dropOrAutoLootItem(Player player, IntIntHolder holder, boolean isRegularItem) {
        if (isRegularItem) {
            ...
            if (isRaidBoss()) {
                broadcastPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_DIED_DROPPED_S3_S2)
                    .addCharName(this).addItemName(holder.getId()).addNumber(holder.getValue()));
            }
        } else {
    ...
    /**
     * Drop a reward on the ground, to this {@link Monster} feet. It is item protected to the {@link Player} set as
     * parameter.
     *
