...
public class GraveyardManager implements Runnable {
    ...
    private void onDie(OnDie event) {
        ...
        if (event.getVictim() instanceof Player player) {
            if (validateDeath(player, event.getKiller(), event.getReason())) {
                // add deadman to queue for future delete
                addDeadMan(event);
            }
        }
    }
    ...
    private boolean validateDeath(Player player, Creature killer, DieReason reason) {
        ...
        else if (!reason.isHardcoreDeath()) {
            return false;
        }
    ...
    public void run() {
        ...
        necrologues.values().forEach(necrologue -> {
            ...
            if (player == null || !player.isOnline()) {
                deleteOffline(necrologue.objectId, necrologue.name, necrologue.clanId);
            } else {
                deleteOnline(player);
            }
    ...
    private void deleteOffline(int objectId, String name, int clanId) {
        LOGGER.info("[5][GraveyardManager.deleteOffline] {}, {}, {}", objectId, name, clanId);
        Clan clan = ClanTable.getInstance().getClan(clanId);
        if (clan == null) {
            throw new NullPointerException("Trying to delete clan which is not exist in ClanTable. Player={}." + objectId);
        }
    ...
    private String tryGenerateMessage(String playerName, String killerName, DieReason reason, String zoneName) {
        ...
        return switch (reason) {
             ...
             */
            case MONSTER -> builder.append(playerName)
                .append(" стал жертвой кровожадного ")
                .append(killerName).append(" недалеко от ").append(zoneName).append(", ")
             ...
             */
            case DROWN ->
                builder.append(playerName).append(" не рассчитал свои силы и захлебнулся, утонув в водах ").append(zoneName).append(".<br>")
             ...
             */
            case FALL ->
                builder.append(playerName).append(" разбился упав с высоты на просторах ").append(zoneName).append(".<br>")
                    .append("Всегда нужно смотреть под ноги и осознавать, что - природа, может убивать не хуже меча!").toString();
            /*
             ...
             */
            case GUARD -> builder.append(HtmColor.WHITE.asColored(playerName)).append(" нарушил закон окрестностей ")
                .append(HtmColor.WHITE.asColored(zoneName)).append(", за что и поплатился!<br>")
                .append("Приговор в исполнение привел ").append(killerName).append(".<br>")
