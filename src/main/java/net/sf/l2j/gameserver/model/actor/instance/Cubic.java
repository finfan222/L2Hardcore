...
public class Cubic {
    ...
    /**
     * Stop entirely this {@link Cubic} action (both action/disappear tasks are dropped, id is removed from
     * {@link Player} owner. Enforce broadcast.
     *
     * @see #stop(boolean)
     */
    public void stop() {
    ...
    /**
     * Fire the action associated to this {@link Cubic} id. If the owner is dead or offline, stop the {@link Cubic}
     * entirely.
     */
    private void fireAction() {
        ...
        if (_id == LIFE_CUBIC) {
            ...
            _castTask = ThreadPool.schedule(() -> skill.useSkill(_owner, targets), CAST_DELAY);
            ...
            _castTask = ThreadPool.schedule(() ->
            {
                switch (skill.getSkillType()) {
                    ...
                    case DRAIN:
                        useDrainSkill((Drain) skill, target);
                        break;
    ...
    private void useDisablerSkill(L2Skill skill, Creature target) {
        ...
        if (skill.getSkillType() == SkillType.AGGDAMAGE) {
            ...
            if (target instanceof Player && ((Player) target).isInDuel() && skill.getSkillType() == SkillType.DEBUFF && getOwner().getDuelId() == ((Player) target).getDuelId()) {
                for (AbstractEffect effect : skill.applyEffects(getOwner(), target)) {
    ...
    private void useDrainSkill(Drain skill, Creature target) {
