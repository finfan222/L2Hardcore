    ...
    public boolean canDoAttack(Creature target) {
        ...
        if (target instanceof Playable) {
            if (attacker.isInsideZone(ZoneId.PEACE)) {
                attacker.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.CANT_ATK_PEACEZONE));
                return false;
            }

            if (target.isInsideZone(ZoneId.PEACE)) {
                attacker.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.TARGET_IN_PEACEZONE));
                return false;
            }
        }
