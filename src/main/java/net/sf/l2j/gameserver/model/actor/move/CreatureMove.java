...
public class CreatureMove<T extends Creature> {
     ...
     * @param pathfinding : If true, we try to setup a pathfind. If a pathfind occured, then the destination
     * {@link Location} is the first pathfind segment.
     */
     ...
     * <B><U> Concept</U> :</B><BR>
     * <BR>
     * At the beginning of the move action, all properties are stored. The position of the start point and of the
     * destination permit to estimated in function of the movement speed the time to achieve the destination.<BR>
     * <BR>
     * When the movement is started, this method will be called each 0.1 sec to estimate and update the Creature
     * position on the server. Note, that the current server position can differe from the current client position even
     * if each movement is straight foward. That's why, client send regularly a Client->Server ValidatePosition packet
     * to eventually correct the gap on the server. But, it's always the server position that is used in range
     * calculation.<BR>
     * <BR>
     * At the end of the estimated movement time, the Creature position is automatically set to the destination position
     * even if the movement is not finished.
     *
     * @param firstRun
    ...
    /**
     * Create and launch a follow task upon a {@link WorldObject} pawn, executed every 1s. It is used by
     * onIntentionFollow.
     *
