 ...
 * <br>
 * In order to participate, a {@link Player} has to pick the quest from Dimensional Gate Keeper. Killing monsters in
 * Seven Signs reward him with Dimensional Fragments, which can be used to enter the Rifts.<br>
 * <br>
 * A {@link Party} of at least 2 members is required. There are 6 different Rifts based on level range, accessible by
 * the given NPC.<br>
 * <br>
 * Once Dimensional Fragments are consumed, the Party is moved into the first {@link DimensionalRiftRoom}. Monsters
 * spawn after 10sec. Party leader got the possibility to use a free-token to change of room, but only once. The Party
 * automatically jumps of room in an interval between 480 and 600sec. An earthquake effect happens 7sec before the
 * teleport.<br>
 * <br>
 * In the Rift, one of the cell is inhabited by Anakazel, a Raid Boss. The cell can only be reached through regular
 * teleport ; this cell can't be the first one, nor a free-token one.<br>
 * <br>
 * The Dimensional Rift collapses after 4 jumps, or if the party wiped out. Players using teleport (SoE, unstuck,...)
 * are moved back to Dimensional Waiting Room. If the amount of Players inside the Rift is reduced to 1, then the Rift
 * collapses. Any Party edit (add/remove party member) also produces the Rift to collapse.
 */
public class DimensionalRift {
    ...
    /**
     * The manual teleport, consisting to {@link Player} using the free token on the teleporter {@link Npc} to freely
     * change of room. Only one token is normally allowed.
     *
    ...
    /**
     * Allow a {@link Player} to manually exit the {@link DimensionalRift}. He needs to be party leader. If successful,
     * the DimensionalRift is destroyed.
     *
     * @param player : The tested Player.
     * @param npc : The called Npc.
     */
    ...
    /**
     * On {@link Player} teleport (SoE, unstuck,...), put the Player on revived {@link List}. If the minimum required
     * party members amount isn't reached, we kill this {@link DimensionalRift} instance.
     *
