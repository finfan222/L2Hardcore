    ...
    public void onBypassFeedback(Player player, String command) {
        if (command.startsWith("AskWedding")) {
            ...
            if (st.hasMoreTokens()) {
                ...
                ConfirmDlg confirmDlg = new ConfirmDlg(1983)
                    .addString(player.getName() + " asked you to marry. Do you want to start a new relationship ?")
                    .addRequesterId(player.getObjectId());
                partner.setDialog(new Dialog(partner, confirmDlg, Collections.emptyMap()).send());
            } else {
                sendHtmlMessage(player, "data/html/mods/wedding/notfound.htm");
            }
